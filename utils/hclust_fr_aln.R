...
if (args.mode == "MSA") {
	...
	m<<-as.matrix(d)^2
}else if (args.mode == "PW") {
	m<<-read.table(args.input_file,h=T)
}
...
for (dist in c(0.005,1:10/100)) {
    ...
    write.table(df, file=paste0(args.output_dir,"/",args.mode,".",args.algo,".",dist,".hclust"), col.names=T, row.names=F, quote=F)
}
